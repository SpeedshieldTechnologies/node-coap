...
var dgram = require('dgram')
    ...
    , LOG = require("@speedshield/velocity-logging").LogFactory.getLogger("Node-Coap-Server.js")
...
CoAPServer.prototype._handle = function (packet, rsinfo) {
    ...
    var cache_key = toCacheKeyCustom(rsinfo.address, rsinfo.port, packet);
    var cache_key_block1 = toCacheKeyCustom(rsinfo.address, rsinfo.port, packet);
    ...
    if (packet.token && packet.token.length > 0) {
        ...
        if (block2Buff) {
            ...
            LOG.debug(`Block2 - cache_key=${cache_key}. Block number: ${requestedBlockOption.num}`);
            ...
            if (requestedBlockOption.num < 1) {
                if (this._block2Cache.remove(cache_key)) {
                    ...
                    LOG.debug('first block2 request, removed old entry from cache');
                }
            } else {
                debug('check if packet token is in cache, key:', cache_key)
                LOG.debug(`check if packet token is in cache, key: ${cache_key}`);
                if (this._block2Cache.contains(cache_key)) {
                    debug('found cached payload, key:', cache_key)
                    LOG.debug(`found cached payload, key: ${cache_key}`);
                    response.end(this._block2Cache.get(cache_key));
                    return;
                } else {
                    LOG.debug(`Cache key ${cache_key} NOT found`);
                }
            }
        } else {
            ...
            if (block1Buff) {
                ...
                if (blockState) {
                    ...
                    LOG.debug(`block1 cache_key : ${cache_key_block1} - Sequence number ${blockState.sequenceNumber}`);
                    var cachedData = this._block1Cache.getWithDefaultInsert(cache_key_block1)
                    ...
                    if (!blockState.moreBlocks) {
                        ...
                        LOG.debug(`block1 cache_key : ${cache_key_block1} - last block - ByteOffsets ${byteOffsets}`);

                        var byteTotalSum = incomingByteIndex + request.payload.length

                        LOG.debug(`Total byte sum ${byteTotalSum}`);
                        ...
                        for (var i = 0; i < byteOffsets.length; i++) {
                            if (byteOffsets[i] == next) {
                                ...
                                this.emit("error", "Byte offset not the next in line...");
                                throw new Error("Byte offset not the next in line...");
                            }
                        }

                        this._block1Cache.remove(cache_key_block1)

                        if (next == concat.length) {
                            request.payload = concat
                        } else {
                            this.emit("error", "Last byte index is not equal to the concat buffer length!");
                    ...
                    this.emit("error", "Invalid block state " + blockState);
                    throw new Error("Invalid block state : " + blockState);
                }
            }
        }
    } else {
        this.emit('error', "Packet must contain a token. " + JSON.stringify(packet));
        throw new Error("Packet must contain a token");
    }

    LOG.debug("Sending a new request to CoAPTransportReceiver");
...
function toCacheKeyCustom(address, port, packet) {
    ...
    if (packet.token && packet.token.length > 0) {
        ...
        if (options && options.length > 0) {
            ...
            return packet.token.toString('hex') + '/' + paths.join('/');
        }
    }

    // if device id is NOT found then use this method
    toCacheKey(address, port, packet);
}
